...
public static class EntClaimsIdentityExtensions
{
    public static Guid? FindUserId([NotNull] this ClaimsPrincipal principal)
    {
        EntCheck.NotNull(principal, nameof(principal));

        var userIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.UserId);
    ...
    {
        EntCheck.NotNull(identity, nameof(identity));

        var claimsIdentity = identity as ClaimsIdentity;

        var userIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.UserId);
        if (userIdOrNull == null || userIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    public static Guid? FindTenantId([NotNull] this ClaimsPrincipal principal)
    {
        ...
        var tenantIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.TenantId);
    ...
    public static Guid? FindTenantId([NotNull] this IIdentity identity)
    {
        ...
        var tenantIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.TenantId);
        if (tenantIdOrNull == null || tenantIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    public static string? FindClientId([NotNull] this ClaimsPrincipal principal)
    {
        ...
        var clientIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ClientId);
    ...
    public static string? FindClientId([NotNull] this IIdentity identity)
    {
        ...
        var clientIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ClientId);
        if (clientIdOrNull == null || clientIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    public static Guid? FindEditionId([NotNull] this ClaimsPrincipal principal)
    {
        ...
        var editionIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.EditionId);
    ...
    public static Guid? FindEditionId([NotNull] this IIdentity identity)
    {
        ...
        var editionIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.EditionId);
        if (editionIdOrNull == null || editionIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    public static Guid? FindImpersonatorTenantId([NotNull] this ClaimsPrincipal principal)
    {
        ...
        var impersonatorTenantIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ImpersonatorTenantId);
    ...
    public static Guid? FindImpersonatorTenantId([NotNull] this IIdentity identity)
    {
        ...
        var impersonatorTenantIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ImpersonatorTenantId);
        if (impersonatorTenantIdOrNull == null || impersonatorTenantIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    public static Guid? FindImpersonatorUserId([NotNull] this ClaimsPrincipal principal)
    {
        ...
        var impersonatorUserIdOrNull = principal.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ImpersonatorUserId);
    ...
    public static Guid? FindImpersonatorUserId([NotNull] this IIdentity identity)
    {
        ...
        var impersonatorUserIdOrNull = claimsIdentity?.Claims?.FirstOrDefault(c => c.Type == EntClaimTypes.ImpersonatorUserId);
        if (impersonatorUserIdOrNull == null || impersonatorUserIdOrNull.Value.IsNullOrWhiteSpace())
        {
            return null;
        }
    ...
    {
        EntCheck.NotNull(claimsIdentity, nameof(claimsIdentity));
